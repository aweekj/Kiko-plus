...
I'm stil a beginner in all things Docker, this is a record of my first stab at creating a working development setup on osx. Please let me know what you think in the comments below.
...
Luckily the process has been simplified significantly, just download and install the Docker app from the [Docker download page](https://www.docker.com/products/docker#/mac "download Docker") and you'll get 

![Docker is ready]({{site.baseurl}}/images/Screen Shot 2017-01-06 at 14.07.54.png)

Also see the [Get Started with Docker for Mac](https://docs.docker.com/docker-for-mac/) guide.
...
|||
|-|-|
|**Dockerfile**|A textfile containing the instructions to build a Docker image.|
|**Image**|The result of runnng your `Dockerfile`, a blueprint for a container,ready to be instantiated and shared.|
...
To parse the commandline arguments, we use a library called [Click](click.pocoo.org). To use it we'll add it to `requirements.txt`.
...
Even better yet, there are specialised images that come preinstalled with a specific Python version! Let's use the [Alpine Python 3.6](https://hub.docker.com/_/python/) image for this application and finish the `Dockerfile`.
...
We can clean up these container by noting the `CONTAINER ID` and running:
